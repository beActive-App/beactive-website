...
// TODO: replace with the real App Store listing URL once BeActive is approved and live.
const APP_STORE_URL = '#'
...
const screenshots = [
  { src: '/screenshots/1-meine-events.png', alt: 'Meine Events – alle erstellten und beigetretenen Events im Überblick' },
  { src: '/screenshots/2-event-details.png', alt: 'Event-Details – beitreten, chatten und teilen' },
  { src: '/screenshots/3-erstellen.png', alt: 'Event erstellen – in wenigen Sekunden veröffentlicht' },
  { src: '/screenshots/4-profil.png', alt: 'Profil – Interessen, Kontakte und Einstellungen' },
]
...
const faqs = [
  ...
  {
    q: 'Wo kann ich die App herunterladen?',
    aBefore: 'BeActive ist kostenlos für iPhone erhältlich. Tippe oben auf ',
    aLinkText: '„App laden“',
    aAfter: ' oder besuche den App Store direkt – eine Android-Version ist aktuell nicht verfügbar.',
    href: APP_STORE_URL,
  },
  {
    ...
    aAfter: ' auf dieser Seite zeigen, wie das aussieht – weitere echte Partner-Deals kommen laufend dazu.',
    href: '#deals',
  },
  {
      ...
      <div className={styles['faq-answer']}>
        <p>
          {'aLinkText' in item ? (
            <>
              {item.aBefore}
              <a href={item.href} className={styles['faq-inline-link']}>
                {item.aLinkText}
              </a>
              {item.aAfter}
            </>
...
function DownloadBadge() {
  return (
    <a href={APP_STORE_URL} className={styles['download-badge']} aria-label="Im App Store laden">
      <svg className={styles['apple-mark']} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d="M16.365 1.43c0 1.14-.468 2.096-1.235 2.85-.767.754-1.79 1.29-2.914 1.207-.14-1.1.4-2.24 1.16-2.98.78-.77 2.02-1.33 2.99-1.077zM20.85 17.19c-.51 1.15-.99 2.24-1.75 3.28-.98 1.34-2 2.68-3.56 2.71-1.53.03-2.02-.9-3.77-.9-1.75 0-2.29.87-3.75.93-1.5.06-2.66-1.45-3.65-2.78-1.98-2.7-3.5-7.62-1.47-10.94.93-1.63 2.6-2.66 4.4-2.69 1.48-.03 2.86.99 3.76.99.89 0 2.58-1.22 4.35-1.04.74.03 2.82.3 4.16 2.27-3.79 2.31-3.19 6.81.24 8.16z" />
      </svg>
      <span className={styles['label-lines']}>
        <span className={styles['eyebrow-line']}>Download on the</span>
        <span className={styles['store-line']}>App Store</span>
      </span>
    </a>
  )
}

export default function LaunchedHomePage() {
      ...
      <header className={cx(styles.nav, scrolled && styles.scrolled)}>
        <div className={cx(styles.wrap, styles['nav-row'])}>
          ...
          <div className={styles['nav-right']}>
            <a href={APP_STORE_URL} className={styles.btn}>App laden</a>
            <button
              type="button"
      ...
      <main>
        <section className={styles.hero} id="top" ref={heroRef}>
          ...
          <div className={styles.wrap}>
            <span className={styles.eyebrow}>Jetzt verfügbar</span>
            ...
            <div className={styles['hero-actions']}>
              <a href={APP_STORE_URL} className={styles.btn}>Jetzt kostenlos laden</a>
            </div>
          </div>
          <div className={styles['arch-frame']} aria-hidden="true" />
        </section>

        <section className={cx(styles.section, styles['screens-section'])} id="app">
          <div className={styles.wrap}>
            <Reveal className={styles['section-head']}>
              <span className={styles.eyebrow}>So sieht’s aus</span>
              <h2 style={{ marginTop: 14 }}>Deine App, ein Blick</h2>
            </Reveal>
            <div className={styles['screens-gallery']}>
              {screenshots.map((s) => (
                <Image key={s.src} src={s.src} alt={s.alt} width={1320} height={2868} loading="lazy" />
              ))}
            </div>
          </div>
        </section>
          ...
          <div className={styles.wrap}>
            <Reveal className={styles['deals-head']}>
              ...
              <div>
                ...
                <p className={styles['deals-note']}>Beispielhafte Darstellung – echte Partner-Deals kommen laufend dazu.</p>
              </div>
            </Reveal>
        ...
        <section className={styles.cta} id="kontakt">
          <div className={styles.wrap}>
            <Reveal>
              <span className={styles.eyebrow}>Bereit?</span>
              <h2 style={{ marginTop: 18 }}>Hol dir BeActive jetzt</h2>
              <p>Kostenlos im App Store – auf iPhone verfügbar.</p>
